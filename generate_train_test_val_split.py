...
alt_set = False  # create an alternative split
...
# Checking whether reward rate is lower in validation sessions, but not really...
# import matplotlib.pyplot as plt
# train_rewards = []
# for eid in train_eids:
#     train_rewards.append(((mice_data[mice_data.session == eid].feedbackType + 1) / 2).mean())

# val_rewards = []
# for eid in validate_eids:
#     val_rewards.append(((mice_data[mice_data.session == eid].feedbackType + 1) / 2).mean())

# plt.hist(train_rewards, density=True, alpha=0.4)
# plt.hist(val_rewards, density=True, alpha=0.4)
